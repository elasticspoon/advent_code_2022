...
class DaySeven
  ...
  def change_directory(path)
    @current_directory = if path == '..'
                           current_directory.parent
                         elsif path == '/'
                           @head
                         else
                           current_directory.find { |item| item.name == path }
                         end
  end
  ...
  def populate_directory(input)
    ...
    @current_directory << if file_size == 'dir'
                            FileDirectory.new(file_name, current_directory)
                          else
                            ElfFile.new(file_name, file_size.to_i)
                          end
  end
...
class FileDirectory
  ...
  def size_smaller_than(size)
    ...
    self.size < size ? self.size + subd_size : subd_size
  end

  def dir_sizes
    [size] + @items.map(&:dir_sizes).flatten
  end
...
File.open('day_seven_input.txt') do |file|
  ...
  puts computer.clean_space(70_000_000, 30_000_000)
end
