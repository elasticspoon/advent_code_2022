...
RSpec.describe DayFive do
  describe '#set_crate_state' do
    ...
    xit 'readed in the example input correctly' do
      ...
      expect(described_class.set_crate_state(test_input)).to eq([%w[D N Z], %w[C M], ['P']])
    end
  end

  describe '#do_operation' do
    let(:boat) { described_class.new(nil) }
    let(:initial_state) { [%w[D N Z], %w[C M], ['P']] }
    ...
    it 'moves 1 crate corretly' do
      ...
      expect(boat.crate_state).to eq([%w[C D N Z], ['M'], ['P']])
    end

    it 'moves 2 crate corretly' do
      boat.do_operation('move 2 from 2 to 1')
      expect(boat.crate_state).to eq([%w[M C D N Z], [], ['P']])
    end
  end
  ...
  describe '#set_operations' do
    ...
    it 'returns the operations' do
      expect(described_class.get_operations(test_input)).to eq(
        [
          'move 1 from 9 to 2',
          'move 4 from 6 to 1',
          'move 2 from 4 to 3'
